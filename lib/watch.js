...
Watch = (function() {
  ...
  Watch.abort = function(app, pack, part) {
    ...
    for (i = 0, len = ref.length; i < len; i++) {
      task = ref[i];
      kill = false;
      if ((app != null) && (task.app == null)) {
        return this._kill();
      } else if ((app != null) && (pack != null) && (task.pack == null)) {
        return this._kill(app);
      } else if ((app != null) && (pack != null) && (part != null) && (task.part == null)) {
        return this._kill(app, pack);
      }
      if ((app == null) || (task.app === app)) {
        kill = true;
      } else if (((app != null) && (pack == null)) || (task.app === app && task.pack === pack)) {
        kill = true;
      } else if (((app != null) && (pack != null) && (part == null)) || (task.app === app && task.pack === pack && task.part === part)) {
        kill = true;
      }
      try {
        if (kill) {
  ...
  Watch._watchConfig = function() {
    ...
    fs.watch(path.relative(process.cwd(), 'easycompile.json'), (function(_this) {
      return function() {
        debug('Config change');
        _this.abort();
        _.each(_this._configWatches, function(watch) {
  ...
  Watch._loadTemplateWatches = function*(app, pack) {
    var _app, _pack, appConfig, packConfig, results, results1;
    ...
    for (_app in results) {
      ...
      if ((app == null) || app === _app) {
        results1.push((function() {
          var ref, ref1, results2;
          ref = appConfig.javascripts;
          results2 = [];
          for (_pack in ref) {
            packConfig = ref[_pack];
            if ((pack == null) || pack === _pack) {
              if ((ref1 = packConfig.templates) != null ? ref1.length : void 0) {
                results2.push(this._watch(_app, _pack, _.map(packConfig.templates, (function(_this) {
                  return function(file) {
                    return _this.config[_app]['javascripts'].webRoot + file;
                  };
                })(this))));
              } else {
  ...
  Watch._watch = function(app, pack, files) {
    ...
    fswatcher.on('all', (function(_this) {
      return function(event, file) {
        ...
        return _this._startCompile(app, pack, 'template');
      };
    })(this));
