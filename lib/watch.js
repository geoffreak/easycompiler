...
Watch = (function() {
  ...
  Watch.abort = function(app, pack, part) {
    ...
    if ((app != null) && !_.isArray(app)) {
      app = [app];
    }
    if ((pack != null) && !_.isArray(pack)) {
      pack = [pack];
    }
    if ((part != null) && !_.isArray(part)) {
      part = [part];
    }
    ref = this._tasks;
    for (i = 0, len = ref.length; i < len; i++) {
      task = ref[i];
      if (!app.length || task.app.join(',') !== app.join(',')) {
        app = _.uniq(app.concat(task.app));
      }
      if (!pack.length || task.pack.join(',') !== pack.join(',')) {
        pack = _.uniq(pack.concat(task.pack));
      }
      if (!part.length || task.part.join(',') !== part.join(',')) {
  ...
  Watch._startCompile = function(app, pack, part) {
    ...
    fork = child.fork(path.resolve(__dirname, 'watchRun.js'), [], {
      env: _.defaults({
        app: (app != null ? app.join(',') : void 0) || '',
        pack: (pack != null ? pack.join(',') : void 0) || '',
        part: (part != null ? part.join(',') : void 0) || ''
      }, process.env)
    });
    task = {
      fork: fork,
  ...
  Watch._loadTemplateWatches = function*(app, pack) {
    ...
    for (_app in results) {
      ...
      if (!(!(app != null ? app.length : void 0) || app.indexOf(_app) !== -1)) {
        continue;
      }
      ref = appConfig.javascripts;
      for (_pack in ref) {
        packConfig = ref[_pack];
        if (!(pack != null ? pack.length : void 0) || pack.indexOf(_pack) !== -1) {
      ...
      return function(filename) {
        ...
        fn = function*() {
          ...
          try {
            ...
            return _this._startCompile(app, (contained.length === 1 ? contained[0] : null), type);
          } catch (_error) {
            e = _error;
